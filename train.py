...
for epoch in range(1, NUM_ADV_EPOCHS + 1):
    ...
    #################
    res = trainer.train(train_dataloader, gen_dataloader=gen_dataloader, epoch=epoch, adversarial=True)
    #################
    ...
    epoch_metrics.update(
        {'epoch': NUM_STD_EPOCHS + epoch, 'lr': last_lr, 'test_clean_acc': test_acc, 'test_adversarial_acc': ''})

    if epoch % args.adv_eval_freq == 0 or epoch > (NUM_ADV_EPOCHS - 5) or (
            epoch >= (NUM_ADV_EPOCHS - 10) and NUM_ADV_EPOCHS > 90):
        test_adv_acc = trainer.eval(test_dataloader, adversarial=True)
